...
vector<int> GetFail(const string &p) // 0-based
{
    int m=p.size();
    vector<int> fail(m, 0); // 0-based
    fail[0]=0; // 초기 조건
...
vector<int> KMP(const string &s, const string &p)
{
    ...
    for(int i=0, j=0; i<n; i++) //i = s에서 비교할 인덱스, j = p에서 비교할 인덱스
    {
        ...
        if(s[i]==p[j])
        {
            if(j+1==m) res.push_back(i-m+1), j=fail[j]; // p의 마지막 문자까지 매칭 완료
            else j++;
        }
    }
    return res;
...
실패함수만 다룬다.
*/ 
vector<int> GetFail(const string& p) // 1-based
{
    int m = p.size();
